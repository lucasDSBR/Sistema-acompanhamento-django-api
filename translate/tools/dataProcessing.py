...
def tokenizeData(dataFile):
    ...
    #translateSaveWords(textTokenized)
    return data
    ...
    for fullParagraphs in textTokenized:
        ...
        for word in paragraphTokenized:
            if Words.objects.filter(ptBR=word):
                ...
                paragraphTokenizedTranslated.append(ts.google(word, from_language='pt', to_language='en'))
                word = Words(
                    ptBR = paragraphTokenized[i],
                    enUS = paragraphTokenizedTranslated[i]
                )
                word.save()
                i = i+1